...
try {
    ...
    & '.\Cbm-Compose.ps1' exec -T cbm-postgres pg_isready -U cbm_app -d cbm_demo
    if ($LASTEXITCODE -ne 0) { throw 'The existing CBM PostgreSQL database is not ready.' }
    $backup = Join-Path $PSScriptRoot ('backups\intake-database-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
    New-Item -ItemType Directory -Path $backup | Out-Null
    # pg_dump plain SQL is UTF-8 text. Preserve the output before applying changes.
    $dump = & '.\Cbm-Compose.ps1' exec -T cbm-postgres pg_dump -U cbm_app -d cbm_demo --no-owner --no-privileges
    ...
    foreach ($sql in @('/docker-entrypoint-initdb.d/02-dispatch.sql','/docker-entrypoint-initdb.d/07-intake.sql','/docker-entrypoint-initdb.d/10-wf2-strict-closure.sql','/docker-entrypoint-initdb.d/11-technician-portal.sql')) {
        & '.\Cbm-Compose.ps1' exec -T cbm-postgres psql -X -U cbm_app -d cbm_demo -v ON_ERROR_STOP=1 -f $sql
    ...
    & '.\Cbm-Compose.ps1' cp '.\database\intake\schema_configuration_pause.sql' 'cbm-postgres:/tmp/cbm-configuration-pause.sql'
    if ($LASTEXITCODE -ne 0) { throw 'Could not stage the configuration-pause migration.' }
    & '.\Cbm-Compose.ps1' exec -T cbm-postgres psql -X -U cbm_app -d cbm_demo -v ON_ERROR_STOP=1 -f /tmp/cbm-configuration-pause.sql
