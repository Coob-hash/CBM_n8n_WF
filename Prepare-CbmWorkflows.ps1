...
& node (Join-Path $PSScriptRoot 'cbm\app\wf2\apply-strict-closure.cjs') (Join-Path $PSScriptRoot 'cbm\app\n8n_wf2_completion_approval_ifc_update.json') (Join-Path $PSScriptRoot 'cbm\app\wf2\workflows\log_ifc_maintenance.json') (Join-Path $PSScriptRoot 'cbm\app\wf2\workflows\notify_fm.json')
...
if (-not $SettingsFile) { $SettingsFile = Join-Path $PSScriptRoot 'cbm\demo.local.json' }
...
$domainLine = Get-Content -LiteralPath (Join-Path $PSScriptRoot '.env') | Where-Object { $_ -match '^\s*NGROK_DOMAIN=' } | Select-Object -First 1
...
$app = Join-Path $PSScriptRoot 'cbm\app'
$out = Join-Path $PSScriptRoot 'cbm\workflows-configured'
...
$replacements = @{}
$installedIds = @{}
$installedMap = Join-Path $PSScriptRoot 'cbm\imported-workflow-ids.json'
