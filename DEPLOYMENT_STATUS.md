...
See [READ-ME-SINGLE-TICKET.md](READ-ME-SINGLE-TICKET.md) for the 16 September single-ticket source draft and the remaining ticket-database binding issue. The record below describes the earlier base release and is historical.
...
No real email was sent and no live MultiSet query was made. The new application workflows have not been imported or published in your n8n. `cbm/demo.local.json` is not present: configure the FM email and incoming/completed Drive folder IDs, run `Prepare-CbmWorkflows.ps1`, then import, bind credentials and publish the new set. IT defaults to **giuseppe.desiderio123@gmail.com**. Existing n8n workflows remain as they were.
