...
$backup = Join-Path $PSScriptRoot 'validation\before-deployment-files'
$files = @('Apply-CbmCompatibility.ps1','Prepare-CbmWorkflows.ps1','docker-compose.yml','cbm\imported-workflow-ids.json','cbm\app\wf1_ticket_intake_and_dispatch.json')
foreach ($folder in @('cbm\dispatch_queue','cbm\app\phase_b\workflows')) {
    $files += Get-ChildItem -LiteralPath (Join-Path $PSScriptRoot $folder) -File | ForEach-Object { Join-Path $folder $_.Name }
}
...
$installed = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'validation\installed-workflows.json') -Raw | ConvertFrom-Json -Depth 100
...
foreach ($name in @('schema_queue.sql','schema_context.sql')) {
    docker cp (Join-Path $PSScriptRoot "database\dispatch_queue\$name") "n8n_deploy-cbm-postgres-1:/tmp/cbm-dispatch-$name"
...
docker cp (Join-Path $PSScriptRoot 'validation\installed-workflows.json') 'n8n_v1:/tmp/cbm-dispatch-updated-workflows.json'
