...
try {
    ...
    $backup = Join-Path $PSScriptRoot ('backups\before-start-' + $stamp)
    ...
    & '.\Cbm-Compose.ps1' build n8n task-runners ifc-init unlimited-ocr-adapter
    ...
    for ($attempt=0; $attempt -lt 20; $attempt++) {
        & '.\Cbm-Compose.ps1' exec -T cbm-postgres pg_isready -U cbm_app -d cbm_demo | Out-Null
        if ($LASTEXITCODE -eq 0) { break }
        Start-Sleep -Seconds 2
    }
    & '.\Apply-CbmIntakeMigration.ps1'
    ...
    Write-Output 'Run .\Test-Cbm.ps1, then follow the tutorial credential and import steps.'
