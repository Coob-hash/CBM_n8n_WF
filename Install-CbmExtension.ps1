...
if ([IO.Path]::GetFullPath($PSScriptRoot).TrimEnd('\') -eq $target) { throw 'Run the installer from the separate extension package, not from the destination.' }
...
$rootFiles = @('Dockerfile','docker-compose.yml','.dockerignore','.gitignore','Initialize-Cbm.ps1','Cbm-Compose.ps1','Start-Cbm.ps1','Test-Cbm.ps1','Prepare-CbmWorkflows.ps1','Apply-CbmCompatibility.ps1','Set-CbmDemoTechnicians.ps1','README.md','CBM_Demo_Tutorial.md')
$rootFiles += @('Prepare-CaseStudyPhotos.ps1','CASE_STUDY_GUIDE.md','INTAKE_APPROVAL_GUIDE.md','Apply-CbmIntakeMigration.ps1')
foreach ($name in $rootFiles) {
    $from = Join-Path $PSScriptRoot $name
    if (-not (Test-Path -LiteralPath $from)) { throw "Incomplete extension package: $name" }
}
foreach ($name in $rootFiles) { Copy-Item -LiteralPath (Join-Path $PSScriptRoot $name) -Destination (Join-Path $target $name) -Force }
$cbmSource = Join-Path $PSScriptRoot 'cbm'
...
& (Join-Path $target 'Initialize-Cbm.ps1')
...
& (Join-Path $target 'Apply-CbmCompatibility.ps1')
