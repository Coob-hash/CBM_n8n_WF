...
The current simplification removes the second image review and keeps the vision stages together. WF1's name and ID remain unchanged. See [the single-ticket update](READ-ME-SINGLE-TICKET.md). Refresh the n8n editor before editing the saved draft; an old open editor can overwrite the update.
...
See [the dispatch explanation and complete get_context examples](READ-ME-DISPATCH.md). All 14 CBM drafts remain inactive. No real emails were sent and no CBM workflow was published. WF2's separate model configuration was not migrated by this WF1 update.
...
Deployment source: `C:\Users\USER\Desktop\n8n_deploy`. Rerunning `Prepare-CbmWorkflows.ps1` preserves the new model stages and knowledge bindings. It prepares files only; it does not publish workflows or resolve the pending ticket-database choice.
