...
async function main(){
 ...
 for(const f of ['../app/schema.sql','../app/schema_dispatch_functions.sql','../intake/schema_intake.sql','schema_queue.sql','schema_context.sql'])await db.exec(fs.readFileSync(path.join(__dirname,f),'utf8'));
 // Reapplying these additive migrations is supported.
 for(const f of ['schema_queue.sql','schema_context.sql'])await db.exec(fs.readFileSync(path.join(__dirname,f),'utf8'));
