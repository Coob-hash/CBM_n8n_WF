...
.\Install-CbmExtension.ps1
Set-Location -LiteralPath 'C:\Users\USER\Desktop\n8n_deploy'
.\Start-Cbm.ps1
.\Test-Cbm.ps1
```

`Start-Cbm.ps1` runs `Apply-CbmIntakeMigration.ps1` after starting PostgreSQL. The migration first takes a database dump, then adds report/capture/bug/outbox tables and authorization columns. Existing Docker volumes are reused. PostgreSQL initialization scripts do not run again merely because an existing container restarts, which is why an explicit migration is supplied.

For a deployment whose containers have already been updated, run `Apply-CbmIntakeMigration.ps1` directly. It is repeatable and does not reset counters or tickets. Do not rerun the original release-review migration afterward: its old uniqueness predicate predates rejected requests.
...
```powershell
.\Prepare-CaseStudyPhotos.ps1 -ReporterEmail 'YOUR_CONTROLLED_REPORTER_EMAIL' -Photo 'IMG_7911.JPG'
```
...
```powershell
.\Prepare-CaseStudyPhotos.ps1 -ReporterEmail 'YOUR_CONTROLLED_REPORTER_EMAIL' -Photo 'IMG_7915.JPG' -ReportId 'THE_UUID_FROM_THE_RETRY_MESSAGE'
```
