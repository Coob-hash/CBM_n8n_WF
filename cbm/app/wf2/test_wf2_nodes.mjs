...
console.log('\n5. Closure Context - one bounded path for both decisions');
{
  ...
  const approved = await run(code, { input:{data:{approved:true}}, nodes: {
  ...
  const rejected = await run(code, { input:{data:{approved:false}}, nodes: {
    'Set Pending Approval': {approval_id:'approval-1'},
  ...
  const timedOut = await run(code, { nodes: {
    ...
    'Parse Completion Assessment': assessment } });
  check('a missing approval payload is EXPIRED and has no closure objectives',
        timedOut[0].json.decision === 'EXPIRED' && timedOut[0].json.objectives.length===0);
}
