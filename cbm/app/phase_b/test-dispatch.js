...
const db_root=path.resolve(root,'../../database');
const results=[];
...
async function embeddedTool(name,request,failMail=false,conflictOnce=false,provided={}) {
  ...
  const installedIds=JSON.parse(fs.readFileSync(path.join(root,'../imported-workflow-ids.json'),'utf8'));
...
async function main(){
  db=new PGlite();
  await db.exec(fs.readFileSync(path.join(db_root,'schema.sql'),'utf8'));
  await db.exec(fs.readFileSync(path.join(db_root,'schema_dispatch_functions.sql'),'utf8'));
  await db.exec(fs.readFileSync(path.join(db_root,'schema_wf2_completion.sql'),'utf8'));
  await db.exec(fs.readFileSync(path.join(db_root,'schema_release_review.sql'),'utf8'));
  await db.exec(fs.readFileSync(path.join(db_root,'technician_portal/schema.sql'),'utf8'));
  await db.exec(fs.readFileSync(path.join(db_root,'intake/schema_intake.sql'),'utf8'));
  await db.exec(fs.readFileSync(path.join(db_root,'dispatch_queue/schema_queue.sql'),'utf8'));
  await db.exec(fs.readFileSync(path.join(db_root,'dispatch_queue/schema_context.sql'),'utf8'));
