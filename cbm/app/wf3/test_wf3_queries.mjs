...
const sqlFile = (f) => fs.readFileSync(path.join(ROOT, '../../database', f), 'utf8');
...
{
  const toolNames = Object.entries(wf.connections)
    .filter(([, spec]) => (spec.ai_tool || []).some(
      (br) => (br || []).some((e) => e.node === 'FM Dashboard Agent')))
    .map(([name]) => name);
  check('eight tools were found to test', toolNames.length === 8, toolNames.length);
