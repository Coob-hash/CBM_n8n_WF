...
$nodes[0].parameters.jsCode = Get-Content -LiteralPath (Join-Path $RepoRoot 'cbm\extract-report.js') -Raw
...
& node (Join-Path $RepoRoot 'cbm\app\wf2\apply-report-submission.cjs') $workflowPath
if ($LASTEXITCODE -ne 0) { throw 'WF2 PDF submission overlay failed.' }
& node (Join-Path $RepoRoot 'cbm\app\wf2\apply-strict-closure.cjs') $workflowPath (Join-Path $RepoRoot 'cbm\app\wf2\workflows\log_ifc_maintenance.json') (Join-Path $RepoRoot 'cbm\app\wf2\workflows\notify_fm.json')
if ($LASTEXITCODE -ne 0) { throw 'WF2 strict closure overlay failed.' }

$wf1Path = Join-Path $RepoRoot 'cbm\app\wf1_ticket_intake_and_dispatch.json'
$wf1 = Get-Content -LiteralPath $wf1Path -Raw | ConvertFrom-Json -Depth 100
$patch = Get-Content -LiteralPath (Join-Path $RepoRoot 'cbm\case-study\wf1-node-patches.json') -Raw | ConvertFrom-Json -Depth 100
$intake = Get-Content -LiteralPath (Join-Path $RepoRoot 'cbm\intake\workflow-patch.json') -Raw | ConvertFrom-Json -Depth 100
...
& node (Join-Path $RepoRoot 'cbm\dispatch_queue\patch-workflows.js')
if ($LASTEXITCODE -ne 0) { throw 'Single-ticket dispatch overlay failed.' }
& node (Join-Path $RepoRoot 'cbm\openrouter\patch-workflows.js')
