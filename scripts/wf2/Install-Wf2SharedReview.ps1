...
$payload=Join-Path $RepoRoot 'validation-wf2-chat-approval\import.json'
...
Get-Content -Raw -LiteralPath (Join-Path $RepoRoot 'database\wf2\review-mail.sql') | docker exec -i n8n_deploy-cbm-postgres-1 psql -U cbm_app -d cbm_demo -v ON_ERROR_STOP=1 -q
...
foreach($rel in @('docs\guides\TICKET_STATE_REFERENCE.md','cbm\app\wf2\shared_review.py','database\wf2\review-mail.sql','cbm\app\wf2\SHARED_FM_REVIEW.md','cbm\app\wf2\validate_wf2.py','cbm\app\wf2\test_shared_review.py','cbm\app\wf2\check-shared-review.cjs')){
 Copy-Item -LiteralPath (Join-Path $RepoRoot $rel) -Destination (Join-Path $Deployment $rel) -Force
}
