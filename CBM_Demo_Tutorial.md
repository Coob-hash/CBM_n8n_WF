...
Read [CASE_STUDY_GUIDE.md](<C:/Users/USER/Desktop/n8n_deploy/CASE_STUDY_GUIDE.md>) first. It identifies the real IFC, all eleven photos, the audited intrinsics, map `MAP_J964JX6MGEGO`, and the exact Drive-to-maintenance demonstration. The model is now your office IFC2X3, with 13 maintainable assets. No synthetic IFC is generated.

The case-study route identifies the asset automatically using VPS/IFC candidates and the existing vision-model call. Failure requests another photo, up to three replacements after the original, then records a bug and notifies IT. The FM authorizes the maintenance intervention only after identification succeeds; no technician is contacted beforehand. WF2 completion acceptance remains a separate decision. Read [INTAKE_APPROVAL_GUIDE.md](<C:/Users/USER/Desktop/n8n_deploy/INTAKE_APPROVAL_GUIDE.md>) for the new process, SQL migration and retry filenames. The existing case-study volumes are reused. MultiSet credentials and measured map registration remain live configuration tasks.
...
The `Cbm-Compose.ps1` wrapper always supplies both environment files and the original project name. Use it instead of a bare `docker compose` command for this deployment. Do not paste either private environment file into chat or commit it to a repository.
...
C:\Users\USER\Desktop\n8n_deploy\
  ...
  Start-Cbm.ps1                 backed-up build/start procedure
  Cbm-Compose.ps1               Docker Compose wrapper
  Test-Cbm.ps1                  infrastructure checks
  Prepare-CbmWorkflows.ps1      prepares 14 inactive demo exports
  Set-CbmDemoTechnicians.ps1    configures controlled recipients
  cbm\
...
.\Cbm-Compose.ps1 logs --tail 50 n8n task-runners ifc-service knowledge-service cbm-postgres
```
...
```powershell
.\Cbm-Compose.ps1 exec -T ifc-service python -c 'import json,urllib.request; print(json.dumps(json.load(urllib.request.urlopen("http://127.0.0.1:8000/elements")),indent=2))'
```
...
```powershell
.\Cbm-Compose.ps1 exec -T ifc-service python -c 'from pathlib import Path; print(Path("/app/models/active_model.txt").read_text())'
```
...
```powershell
.\Set-CbmDemoTechnicians.ps1 -Email1 'YOUR_FIRST_DEMO_ADDRESS' -Email2 'YOUR_SECOND_DEMO_ADDRESS'
```
...
3. Prepare one original photo with `Prepare-CaseStudyPhotos.ps1 -ReporterEmail 'YOUR_CONTROLLED_REPORTER_EMAIL' -Photo 'IMG_7911.JPG'`. Record its report UUID and upload the prepared copy to Drive.
...
```powershell
.\Cbm-Compose.ps1 ps -a
.\Cbm-Compose.ps1 logs --tail 50 n8n task-runners
.\Cbm-Compose.ps1 logs --tail 50 ifc-service knowledge-service cbm-postgres
.\Cbm-Compose.ps1 restart ifc-service knowledge-service
.\Test-Cbm.ps1
```
...
```powershell
.\Cbm-Compose.ps1 up -d n8n task-runners ifc-service
```
...
For a future image rebuild, use `Start-Cbm.ps1` so a consistent n8n backup is made before the container update. Do not return to the old `latest` Dockerfile. n8n and external runner versions must stay matched. [Runner version requirements](https://docs.n8n.io/deploy/host-n8n/configure-n8n/set-up-task-runners).
...
`Start-Cbm.ps1` backs up n8n and its configuration. It does **not** create a coordinated backup of PostgreSQL and IFC history. Before resetting or moving an established CBM demo, stop new ticket processing and back up those stores together. Otherwise the database can refer to an IFC version absent from the restored disk.
...
- [Deployment quick start](<C:/Users/USER/Desktop/n8n_deploy/README.md>)
- [Active Compose configuration](<C:/Users/USER/Desktop/n8n_deploy/docker-compose.yml>)
- [Runtime check workflow](<C:/Users/USER/Desktop/n8n_deploy/cbm/Runtime-Check.json>)
- [WF2 adaptation](<C:/Users/USER/Desktop/n8n_deploy/cbm/extract-report.js>)
- [Python PDF endpoint](<C:/Users/USER/Desktop/n8n_deploy/cbm/demo_api.py>)
